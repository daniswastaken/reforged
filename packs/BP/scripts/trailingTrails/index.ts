...
**
*************************************************************************
** This file contain the scripts used for the Trailing Trails feature.
** It works by checking wether the player is running or walking. Wether
** the player are on water or on land. Those condition later used to
** determine to spawn the particles on the player feet or not.
*/
...
export function initializeTrailingTrails() {
    system.runInterval(() => {
        for (const player of world.getPlayers()) {
            if (!player.isSprinting || player.isInWater) continue;

            // Ground check: check if there is a block immediately below the player
            ...
            const horizontalLen =
                Math.sqrt(view.x * view.x + view.z * view.z) || 1;
            ...
            const dx =
                -view.x * 1.2 +
                perpX * side * sideSpread +
                (Math.random() - 0.5) * 0.2;
            const dy = 1.2 + Math.random() * 0.4;
            const dz =
                -view.z * 1.2 +
                perpZ * side * sideSpread +
                (Math.random() - 0.5) * 0.2;
